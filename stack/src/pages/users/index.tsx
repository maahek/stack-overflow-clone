...
const index = () => {
 ...
 if (!mounted) return null;
  ...
  if (!users || users.length === 0) {
    return (
      <div className="text-center text-gray-500 mt-4">{t("noUsersFound")}</div>
    );
  }
  return (
    <Mainlayout>
      <div className="max-w-6xl">
        ...
        <div className="mb-6">
          <div className="relative max-w-md">
            ...
            <Input placeholder={t("filterByUser")} className="pl-10" />
          </div>
        </div>
            ...
            <Link key={user._id} href={`/users/${user._id}`}>
              <div className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer">
                ...
                <div className="flex items-center text-sm text-gray-600 mb-3">
                  ...
                  <span>{t("joined")} {new Date(user.joinDate).getFullYear()}</span>
                </div>
              </div>
            </Link>
          ))}
